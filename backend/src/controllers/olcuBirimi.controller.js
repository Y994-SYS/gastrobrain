...
const olcuBirimiController = {
    ...
    async hepsiniGetir(req, res) {
        try {
            const data = await olcuBirimiService.hepsiniGetir();
    ...
    async biriniGetir(req, res) {
        try {
            const data = await olcuBirimiService.biriniGetir(Number(req.params.id));
            res.json({ basarili: true, data });
        } catch (error) {
    ...
    async olustur(req, res) {
        try {
            const data = await olcuBirimiService.olustur(req.body);
    ...
    async guncelle(req, res) {
        try {
            const data = await olcuBirimiService.guncelle(Number(req.params.id), req.body);
            res.json({ basarili: true, data });
        } catch (error) {
            res.status(400).json({ basarili: false, mesaj: error.message });
        }
    ...
    async sil(req, res) {
        try {
            await olcuBirimiService.sil(Number(req.params.id));
