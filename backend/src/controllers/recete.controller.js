...
const receteController = {
    ...
    async hepsiniGetir(req, res) {
        try {
            const data = await receteService.hepsiniGetir();
    ...
    async biriniGetir(req, res) {
        try {
            const data = await receteService.biriniGetir(Number(req.params.id));
            res.json({ basarili: true, data });
        } catch (error) {
    ...
    async maliyetHesapla(req, res) {
        try {
            const data = await receteService.maliyetHesapla(Number(req.params.id));
            res.json({ basarili: true, data });
        } catch (error) {
            res.status(500).json({ basarili: false, mesaj: error.message });
        }
    ...
    async olustur(req, res) {
        try {
            const data = await receteService.olustur(req.body);
    ...
    async guncelle(req, res) {
        try {
            const data = await receteService.guncelle(Number(req.params.id), req.body);
            res.json({ basarili: true, data });
        } catch (error) {
            res.status(400).json({ basarili: false, mesaj: error.message });
        }
    ...
    async sil(req, res) {
        try {
            await receteService.sil(Number(req.params.id));
